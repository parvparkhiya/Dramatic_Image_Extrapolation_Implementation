function [best_translation num_patches]=process_branch(Ig, Igi, Ii,Bgi, scal, rot, refl)
  %computeM where M is transformation matrix
  M=computeM(scal,rot,refl);

  %applying M to a uniform patch
  tranformed_patch=compute_transformed_patch(M,patchszx,patchszy);

  %save feature vectors of all patches in interior of Igi
  for i=1:size(Igi,1)
      for j=1:size(Igi,2)
        if (condition) %if tranformed_patch + (i,j) is contained in Igi
          interior_feature(i,j)=compute_featurevector(tranformed_patch + (i,j));
        else
  ...
  for i=1:size(Ig,1)
    for j=1:size(Ig,2)
      if(i<Bgi(1,1) || j<Bgi(1,2) || i>Bgi(2,1) || j>Bgi(2,2))
        feature_vector(i,j)=compute_featurevector(uniformpatch+(i,j));
        for ii=1:size(Igi,1)
          for jj=1:size(Igi,2)
